...
_([".left.inline",
    ...
    "a#install.a-hover.dark.small[href #|style margin-left:2em] <span.fa.fa-download> v1.2",
    "a#bars.a-hover.dark.large[href #|style margin-left:3em] <span.fa.fa-bars>"]),
    _("a.a-hover.small.dark[href #|style margin-right:1em] <span <span.fa.{{x[0]}}>> {{x[1]}}",{
        menu:[
            ["fa-github","Github"],
            ["fa-code","Tutorial"],
    ...
    }).for("x:menu","#menu.medium.[style float:right].inline")
]).render(App);

_(["#jumbotron.ctr.center.dark",
"h1.mx-8 A <span.violet Unique> Way <br#side> Web <span.fa.fa-code> Development",
"p.medium.grey <b Creating>, Composing and Rendering <b HTML Element> in A Unique Way!",
_(["#getstarted.center.mx-16",
    "a[href #].bold.btn.rds.small.d-hover.bg-violet.mx-8 <span.fa.fa-play> Get Started",
    "a[href #].bold.btn.rds.small.mx-8 <span.fa.fa-download> Download",
    "a.btn-coll.mx-8[href #|style border:1px solid #202020].rds.small.dark <span.fa.fa-code> Try It On <b JS Fiddle>"
])
]).render(App);
    ...
    onCreate:function(){
        ...
        for(i = 0; i < l; i++){
            this.add(_([
                ...
                "h3.dark.small "+br[i][0].toUpperCase(),
                "p.small.grey(!) "+br[i][1]+""
            ]));
        }
    }
