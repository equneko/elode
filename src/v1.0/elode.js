/* 
    Elode.js (Source) - v1.0 (Release: 01-11-2022) 
...
(function(_doc,_win){ 'use strict';
    ...
    */
    ...
    function c_char(a,b){ //find and count character
        ...
        return {str:a.substring(i),len:j};
    }
    ...
    function _element_(a,b){
        ...
        if(e.prop._elode_feature==null)
           e.prop._elode_feature = true;
        ...
        a = query_search(a,qy.expr,function($){ var r = {};
            ...
            if($.k == 'if'){// if expression (if boolean)
                try { r.x = eval(r.v.replace('$',"e.prop.")); }catch(err){ r.x = false; return _error_(err); }
                if(r.x){
	...
	function _component_(a,b){
        ...
        else if(c==null&&el==null)return _element_(a,b);
		
		r.hash = "!elode_"; r.child = {}; r.a = a;
		for(i = 0; i < c.length; i++){
			r.child[r.hash+i] = c[i].substring(1,c[i].length-1);
	...
	function _validate_(a,b,c,d){
        //Check Reactive
        _react_(a);
        var props = a._elode_property!=null ? a._elode_property : null, k, i, x, r;
    ...
    function _property_($el){
        //Elode - Basic Feature (Method) | Useful Functions
        if($el._elode_feature){ //Check if using Elode methods
             ...
             $el._remove = function($){
                //Remove Callback
                if($el.onRemove!=null&&$el.onRemove.constructor==Function)$el.onRemove();
        ...
        $el.prop = function(a){if(a!=null){elode_prop(a,$el,true);$el.react();}};

        //Elode - React Element (JS) | reactivity function
        $el.react = function(){_react_($el);};
    ...
    function _error_(err){ var str = "ELODE_ERROR \""+err+"\"",
        el = _win.Elode('div#elode_error() '+str);
    ...
    _win.Elode = function _ElodeModule_(A,B){ if(A==null&&B==null)return; var el;
        if(typeof A == 'string' && A[0]=='!'){ //Elode - HTML Interpreter (Machine Learning :v)
            ...
            var skip = el.getAttribute('elode'), vdom = {el:[]}, c = el.innerHTML, i; c = c.split("\n");
        ...
        if(typeof A == 'string' && A[0]=='_'){ //Elode - HTML Element Selector (single)
            ...
            if(B._elode_feature!=null)B._elode_feature=false;
                    else B._elode_feature = true;
            elode_prop(B,el); elode_react(el); _react_(el);
            return _property_(el);
        }
        if(typeof A != 'string' && 
            there(A.constructor.toString(),"Element")){ //Elode - HTML Element
            el = A; if(B._elode_feature!=null)B._elode_feature=false;
                    else B._elode_feature = true;
            elode_prop(B,el); elode_react(el); _react_(el);
            return _property_(el);
        }
