...
====================================================================
    ...
    Elode.js (ES Module) - v2.0 (Release: 28-06-2023) 
...
window.Elode = Elode;
export { Elode, app, init, use, hook, map, ref, render, router, xss }

// @ElodeModule - Main Function
function Elode(query, property) {
...
function hook(data) {
    for (var i in data) {
        Elode.__hook__[i.toLowerCase()] = data[i];
    }
    return data;
};

/* @ElodeMap - Manipulate your Array into Component/Element */
function map(root, array, callback, isArray) {
    var _root_ = null;
    ...
    array.filter = (function () {
        ...
        return function () {
            ...
            fun = map(array.root(), fun, array.el, true);
...
function use(tags) {
    ...
    function setUse(x, y) {
        ...
        return window.eval("function " + x + "(){" +
            "var el = Elode('" + y + "'), i, args = arguments, query = '', sp = ' ', " +
            "prop = args[args.length-1].constructor == Object ? args[args.length-1]:null;" +
            "if(prop!=null) el = Elode('" + y + "', prop);" +
            "for(i = 0; i < args.length; i++){" +
            "if(typeof args[i] == 'string' ||typeof args[i]=='number'){ " +
            "if(i==0){" +
            "if(typeof args[i] == 'string'){" +
            " if(args[i][0]=='#'||args[i][0]=='.') sp='';" +
            " el = Elode('" + y + "'+sp+args[i], prop); " +
            "}else{ el = Elode('" + y + " '+args[i], prop); }" +
            "}else{ Elode(args[i]).render(el); }" +
            "}" +
...
function _hook_(target, include) {
    var name = include.name.split(Elode.__hookTag__),
        value = include.value == null ? "" : include.value;

    if (name[0] == "") {
        Elode.__hook__[name[1]](target, value);
    } else {
        eval("Elode.__hook__." + name[0] + "." + name[1] + "(target,value);");
    }
}
    ...
    if (k != null) {
        ...
        for (i = 0; i < k.length; i++) {
            ...
            if (k[i][0] == "@") {
                ...
                if (Elode.__hook__.anim[at] != null)
                    rep = "anim:";
                if (Elode.__hook__.on[at] != null)
                    rep = "on:";
...
function _property_(element) {
    ...
    element.clone = function (prop) {
        ...
        if (element.__query__ != null && element.__property__ != null)
            x = Elode(element.__query__, element.__property__);
        else
    ...
    element.__init__ = function () {
        // Hook Includes
        if (!Elode.__disableHook__) {
            var i, attr = element.attr(), del = [];
            if (attr != null) {
                for (i = 0; i < attr.length; i++) {
                    if (attr[i].name.includes(Elode.__hookTag__)) {
        ...
        if (element.node.length > 0) {
            var i, elchild = element.node;
            for (i = 0; i < elchild.length; i++) {
                if (elchild[i].__init__) elchild[i].__init__();

            }
        }

        if (element.onCreate) element.onCreate();
        return element;
    };
...
/* 
    @ELODE_RENDERING - lifecycle to listening render() procedure
...
function _error_(err) {
    ...
    var str = "ELODE_ERROR \"" + err + "\"",
        ...
        el = Elode('div#elode_error() ' + str); console.log(str);
...
for (var x in HTMLElement.prototype) {
    if (x.substring(0, 2) == "on") {
        ...
        eval("Elode.__hook__.on." + name + " = function(el, data){\n" +
...
for (var x in anims) {
    eval("Elode.__hook__.anim." + x + " = function(el, data){\n" +
