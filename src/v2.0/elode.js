...
(function _ElodeMain_() {
    ...
    */
    ...
    window.Elode.app = function (data, callback) {
        ...
        return {
            ...
            at: function (target, action) {
                return app(data[target], action)[0];
            },

            set: function (target, reverse) {
    ...
    */
    ...
    /* 
        @HOOK - procedure to include functions
    ...
    */
    ...
    /* 
        @ELEMENT - procedure to create HTML Element (DOM)
    ...
    */
    ...
    /* 
        @PROPERTY - include ElodeProperty to HTMLElement
    ...
    */
    function _property_(element) {
            ...
            if (value == null) {
                return {
                    add: function (newValue) {
                        var oldValue = element.css('');
                        return element.attr("style", oldValue + ";" + newValue);
                    },
                    del: function (delValue) {
                        var newValue = element.css('');
        ...
        element.react = function () {
            if (element.init) {
                element.__react__ = element.init();
            }
        ...
        element.__init__ = function () {
            ...
            if (element.childNodes) {
                var i, elchild = element.childNodes;
                for (i = 0; i < elchild.length; i++) {
                    if (elchild[i].__init__) elchild[i].__init__();
                }
            }

            if (element.__property__) {
                var k, objref = element.__property__;
                for (k in objref) {
                    if (element[k].constructor != Function) {
                        if (element.hasOwnProperty(k) && !element.hasOwnProperty("$"+k)) {
                            eval("Object.defineProperty(element, '$" + k + "', {" +
                                "set: function(v){" +
                                "   this." + k + "= v; " +
                                "   this.react();" +
                                "}" +
    ...
    /* 
        @RENDER - procedure to do rendering HTMLElement
    ...
    */
    ...
    /* 
        @RENDERING - lifecycle to listening Elode.render() procedure
    ...
    */
    ...
    /* 
        @REACTIVE - procedure to make reactive HTMLElement
    ...
    */
        ...
        */
        function evaluate(target) {
            ...
            if (r != null) {
                ...
                // Case: using @init() - a custom reactive case
                if(target.init)
                    x = target.__react__;
            }
    ...
    /* 
        @INTERNAL - sensitive elode function from core
    ...
    */
    ...
    /* 
        @UTILITIES - an utilities from elode, that important use.
