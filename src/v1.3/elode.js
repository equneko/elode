...
(function(_doc,_win){ 'use strict';
    ...
    */
    ...
    function _property_($el){ var i;
        ...
        //Another Keyword for Sensitive Keyword
        $el.clone = $el.new; $el.seen = $el.if; $el.each = $el.for;
    ...
    /* Elode App - Build Component Based */
    _win.Elode.app = function(root,node,prop){return _app_(root,node,prop);};
    /* Elode Use - Using Namespace HTML Tag */
    _win.Elode.use = function(tags){
        var i,html="";
        if(arguments.length > 1){
            tags = arguments;
            for(i = 0; i < tags.length; i++){
                _win.Elode.use(tags[i]);
            }
            return;
        }
        if(there(tags,"as")){
    ...
    _win.Elode.render = function(nodes){
        if(arguments.length>1){
            nodes = arguments; var i;
            for(i=0; i < nodes.length; i++){
                _win.Elode.render(nodes[i]);
            }
            return;
        }
